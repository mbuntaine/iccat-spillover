...
#Answer from the regression output: Fleets that catch one or more TAC stock increase their catch by 10.89 metric tonnes, relative to fleets that do not catch any TAC stocks.
...
iccat_df_final_test2 <- iccat_df_final_edit %>% 
  group_by(YearC, FleetGear) %>% 
  mutate(totalCatch = sum(Qty_t)) %>% #Total catch grouped by FleetGear and YearC
  filter(tacr == 1) %>% 
  group_by(YearC, FleetGear) %>% 
...
iccat_df_final_reg <- iccat_df_final_edit %>% 
  ...
  mutate(prop_tacC = if_else(is.na(prop_tacC), 0, prop_tacC))
...
tac_reg2 <- lm(Qty_t ~ tac_any*prop_tacC + YearC + FleetGear + SpeciesStock, data = filter(iccat_df_final_reg, tacr==0))
summary(tac_reg2)
